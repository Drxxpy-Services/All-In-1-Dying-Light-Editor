...
SPAWN_TEMPLATES = {
    ...
    "Craft_InstantColdPack": (279, bytes.fromhex("00840400")),
    "Craft_Painkiller": (284, bytes.fromhex("00840400")),
    ...
    "Craft_Syringe": (293, bytes.fromhex("00840400")),
    "Craft_ZZZZ3_FollowingAlgae": (1529, bytes.fromhex("00341627")),
...
def _read_item_display_names() -> dict[str, str]:
    path = Path(__file__).resolve().with_name("item_display_names.json")
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        return {
            str(identifier): str(name).strip()
            for identifier, name in data.items() if str(name).strip()
        }
    except (OSError, ValueError, TypeError, json.JSONDecodeError):
        return {}


def _humanize_item_name(identifier: str, details: dict[str, object]) -> str:
    if identifier in FRIENDLY_NAMES:
        return FRIENDLY_NAMES[identifier]
    properties = details.get("properties", [])
    raw_name = next((value for name, value in properties if name == "Name"), "")
    token_match = re.search(r"&([^&]+)&", str(raw_name))
    token = token_match.group(1) if token_match else identifier
    token = re.sub(
        r"^(?:Item|Melee|Firearm|Throwable|Craftplan|Craft|Misc|Ammo|Bow|Crossbow)_",
        "", token, flags=re.IGNORECASE,
    )
    token = re.sub(r"_(?:N|Name)$", "", token, flags=re.IGNORECASE)
    token = re.sub(r"^z+_?", "", token, flags=re.IGNORECASE)
...
class DyingLightSave:
    ...
    def spawn_catalog_item(self, identifier: str, category: str, quantity: int) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="dlse_spawn_") as folder:
            ...
            if sampled.returncode or not patch.exists():
                ...
                raise SaveFormatError(
                    "The universal serializer could not decode the current save. "
                    "Restore the newest serializer-valid backup before adding more "
                    "universal items."
                    + (f"\n\nTechnical detail: {detail[:300]}" if detail else "")
                )
    ...
    def spawn_named_inventory_items(
        ...
        with tempfile.TemporaryDirectory(prefix="dlse_named_") as folder:
            ...
            if result.returncode or not patch.exists():
                ...
                raise SaveFormatError(
                    "The serializer could not decode the current save. Restore the "
                    "newest serializer-valid backup before adding more items."
                    + (f"\n\nTechnical detail: {detail[:300]}" if detail else "")
                )
            data = json.loads(patch.read_text(encoding="utf-8"))
            inventory = data["player"]["inventory"]
            ...
            existing = {
                str(item.get("name", "")): item for item in all_items
            }
            used_ids = {int(item.get("id", 0)) for item in all_items}
            next_id = max(used_ids, default=0) + 1
            added = 0
            for identifier in identifiers:
                target_category = SOURCE_CATEGORY_INDEX.get(identifier, "")
                category_donors = [
                    candidate for candidate in normal
                    if SOURCE_CATEGORY_INDEX.get(str(candidate.get("name", "")), "")
                    == target_category
                    and str(candidate.get("name", "")) != identifier
                ]
                if identifier in existing:
                    item = existing[identifier]
                    if category_donors:
                        # Repair records previously created from a generic material
                        # donor while retaining their save-local instance identity.
                        definition_id = (
                            ALL_SPAWN_TEMPLATES[identifier][0]
                            if identifier in ALL_SPAWN_TEMPLATES
                            else int(item.get("id", 0))
                        )
                        repaired = copy.deepcopy(category_donors[0])
                        repaired.update(
                            id=definition_id, name=identifier, quantity=quantity
                        )
                        item.clear()
                ...
                while next_id in used_ids:
                    next_id += 1
                item = copy.deepcopy(
                    category_donors[0] if category_donors else normal[0]
                )
                # Known items use stable game-definition IDs (for example
                # Berserker=303, King=310, Clicker=311). Unknown universal items
                # receive a spare ID because no authoritative mapping exists.
                registry_id = (
                    ALL_SPAWN_TEMPLATES[identifier][0]
                    if identifier in ALL_SPAWN_TEMPLATES else next_id
                )
                item.update(id=registry_id, name=identifier, quantity=quantity)
                normal.append(item)
                used_ids.add(registry_id)
                existing[identifier] = item
                if registry_id == next_id:
                    next_id += 1
                added += 1
            patch.write_text(json.dumps(data), encoding="utf-8")
            result = subprocess.run(
    ...
    def write_safely(self) -> Path:
        ...
        self.validate_with_serializer(encoded)
    ...
    def validate_with_serializer(self, encoded: bytes | None = None) -> None:
        """Require the external serializer to parse the complete staged save."""
        helper = Path(__file__).resolve().with_name("editor.exe")
        if not helper.exists():
            raise SaveFormatError("Save validation requires editor.exe beside this file.")
        with tempfile.TemporaryDirectory(prefix="dlse_validate_") as folder:
            work = Path(folder)
            source = work / "candidate.sav"
            patch = work / "candidate.json"
            source.write_bytes(
                encoded
                if encoded is not None
                else gzip.compress(bytes(self.decoded), compresslevel=6, mtime=0)
            )
            result = subprocess.run(
                [str(helper), "sample", f"--patch={patch}", str(source)],
                capture_output=True, text=True, creationflags=0x08000000,
            ...
            if result.returncode or not patch.exists():
                ...
                raise SaveFormatError(
                    "The staged save failed full serializer validation and was not "
                    "written. Restore the newest serializer-valid backup before "
                    "adding more universal items."
                    + (f"\n\nTechnical detail: {detail}" if detail else "")
                )
...
class SaveEditorApp(tk.Tk):
    def __init__(self) -> None:
        ...
        localized_names = _read_item_display_names()
        self.item_display_names = {
            identifier: localized_names.get(
                identifier,
                _humanize_item_name(identifier, self.item_details.get(identifier, {})),
            )
            for identifier, _category in self.item_catalog
        }
    ...
    def _build_information_pages(self) -> None:
        ...
        for key, (title, description) in definitions.items():
            ...
            if key == "progression":
                ...
                legend_rank_buttons = tk.Frame(
                    preset_body, bg=COLORS["panel_alt"]
                )
                legend_rank_buttons.pack(fill="x", pady=5)
                ttk.Button(
                    legend_rank_buttons, text="LEGEND BONUSES: 32,767",
                    style="Accent.TButton",
                    command=self._max_extreme_legend_bonuses,
                ).pack(side="left", expand=True, fill="x", padx=(0, 5))
                ttk.Button(
                    legend_rank_buttons, text="RESET TO 25",
                    command=self._reset_legend_bonuses,
            ...
            if key == "runtime":
                ...
                runtime_cards = (
                    (
                        ...
                        "save-only Legend multipliers. Generated runtime weapon-definition "
                        "overrides are intentionally excluded from this safe package.",
                        self._install_developer_menu, self._disable_developer_menu,
                    ),
                    (
                        1, "GOD MODE + SURVIVAL RULES",
                        "Installs bounded high health and stamina, instant regeneration, "
                        "no stamina drain and safe fall thresholds. Rebuilt from the "
                        "current Steam file without overflow-prone 32-bit maximums.",
    ...
    def _build_spawner_page(self) -> None:
        ...
        self.spawner_tree = ttk.Treeview(
            table_frame,
            columns=("name", "id", "category", "status"),
            show="headings",
            selectmode="browse",
        )
        self.spawner_tree.heading("name", text="IN-GAME NAME")
        self.spawner_tree.heading("id", text="INTERNAL IDENTIFIER")
        self.spawner_tree.heading("category", text="CATEGORY")
        self.spawner_tree.heading("status", text="SPAWN SUPPORT")
        self.spawner_tree.column("name", width=250, anchor="w")
        self.spawner_tree.column("id", width=300, anchor="w")
        self.spawner_tree.column("category", width=125, anchor="w")
        self.spawner_tree.column("status", width=95, anchor="center")
        ...
        self.craft_bulk_button = ttk.Button(
            actions, text="ADD 999,999 CRAFT COMPONENTS",
            command=self.spawn_all_craft_components,
        )
        self.craft_bulk_button.grid(
    ...
    def _show_selected_item_details(self) -> None:
        ...
        display_name, identifier, category, status = map(str, values)
        ...
        self.detail_name_var.set(display_name)
    ...
    def _refresh_spawner_rows(self) -> None:
        ...
        sorted_catalog = sorted(
            self.item_catalog,
            key=lambda item: (
                self.item_display_names.get(item[0], item[0]).casefold(),
                item[0].casefold(),
            ),
        )
        for identifier, category in sorted_catalog:
            simple_category = category.replace("CategoryType_", "")
            display_name = self.item_display_names.get(identifier, identifier)
            if (
                selected_category
            ...
            if (
                query
                and query not in display_name.casefold()
                and query not in identifier.casefold()
                and query not in simple_category.casefold()
            ):
                continue
            if identifier in existing:
                status = "EXISTING"
            ...
            self.spawner_tree.insert(
                "", "end", values=(display_name, identifier, simple_category, status),
    ...
    def spawn_selected_item(self) -> None:
        if not self.save:
            ...
            return
        if getattr(self, "_craft_spawning", False) or getattr(
            self, "_item_spawning", False
        ):
            return
        selection = self.spawner_tree.selection()
        if not selection:
        ...
        identifier = str(selected_values[1])
        status = str(selected_values[3])
        try:
            ...
            if status == "EXISTING" and category != "CategoryType_CraftPart":
                self.save.spawn_supported_item(identifier, quantity)
            else:
                self._spawn_item_in_background(identifier, quantity)
                return
    ...
    def _spawn_item_in_background(self, identifier: str, quantity: int) -> None:
        if not self.save or getattr(self, "_item_spawning", False):
            return
        active_save = self.save
        self._item_spawning = True
        self._item_result_queue = queue.Queue(maxsize=1)
        self.spawn_button.configure(state="disabled", text="VALIDATING…")
        self.craft_bulk_button.configure(state="disabled")
        self.save_button.configure(state="disabled")

        window = tk.Toplevel(self)
        window.title(f"{APP_TITLE} // ADD ITEM")
        window.configure(bg=COLORS["black"])
        window.resizable(False, False)
        window.transient(self)
        window.protocol("WM_DELETE_WINDOW", lambda: None)
        width, height = 460, 155
        ...
        ttk.Label(
            panel, text="VALIDATING ITEM RECORD", style="Title.TLabel",
            font=("Segoe UI Semibold", 12),
        ).pack(anchor="w")
        ttk.Label(
            panel,
            text=f"{self.item_display_names.get(identifier, identifier)} × {quantity:,}",
            style="Muted.TLabel",
        ).pack(anchor="w", pady=(7, 14))
        progress = ttk.Progressbar(panel, mode="indeterminate")
        progress.pack(fill="x")
        progress.start(10)
        self._item_progress_window = window
        self._item_progress_bar = progress
        self.status_var.set(f"WORKING  •  validating {identifier}…")

        def worker() -> None:
            try:
                working = DyingLightSave(active_save.path, bytes(active_save.decoded))
                working.spawn_named_inventory_items([identifier], quantity)
                working.validate_with_serializer()
                result = ("ok", working, identifier, quantity, active_save)
            except (ValueError, SaveFormatError, OSError, json.JSONDecodeError) as exc:
                result = ("error", exc, identifier, quantity, active_save)
            self._item_result_queue.put(result)

        threading.Thread(target=worker, daemon=True).start()
        self.after(60, self._poll_item_spawn_result)

    def _poll_item_spawn_result(self) -> None:
        try:
            status, payload, identifier, quantity, active_save = (
        ...
        progress = getattr(self, "_item_progress_bar", None)
        window = getattr(self, "_item_progress_window", None)
        if progress is not None:
            progress.stop()
        if window is not None and window.winfo_exists():
            window.destroy()
        self._item_spawning = False
        self.craft_bulk_button.configure(state="normal")
        self.save_button.configure(state="normal")

        if status == "error":
            self._refresh_spawner_rows()
            self.status_var.set(f"FAILED  •  {identifier} was not changed")
            messagebox.showerror(APP_TITLE, str(payload), parent=self)
            return
        if self.save is not active_save:
            self._refresh_spawner_rows()
            self.status_var.set("CANCELLED  •  a different save was opened")
            return
    ...
    def spawn_all_craft_components(self) -> None:
        if not self.save:
            messagebox.showerror(APP_TITLE, "Open a save first.", parent=self)
            return
        if getattr(self, "_craft_spawning", False):
            return
        identifiers = sorted(
            identifier
            for identifier, category in self.item_catalog
            if category == "CategoryType_CraftComponent"
            # This volatile quest sample uses a special record layout despite
            # sharing the source category with ordinary crafting materials.
            and identifier != "misc_voletilesample"
        )
        if not identifiers:
            messagebox.showerror(
                APP_TITLE, "No craft components were found in the source catalogue.",
                parent=self,
            )
            return
        if not messagebox.askyesno(
            APP_TITLE,
            f"Add all {len(identifiers)} source-defined craft components and set "
            f"every stack to {MAX_QUANTITY:,}?\n\n"
            "The changes remain staged until you press SAVE CHANGES.",
            parent=self,
        ):
            return
        active_save = self.save
        self._craft_spawning = True
        self._craft_result_queue = queue.Queue(maxsize=1)
        self.craft_bulk_button.configure(state="disabled", text="ADDING COMPONENTS…")
        self.spawn_button.configure(state="disabled")
        self.save_button.configure(state="disabled")

        window = tk.Toplevel(self)
        window.title(f"{APP_TITLE} // CRAFT COMPONENTS")
        window.configure(bg=COLORS["black"])
        window.resizable(False, False)
        window.transient(self)
        window.protocol("WM_DELETE_WINDOW", lambda: None)
        ...
        y = max(0, (window.winfo_screenheight() - height) // 2)
        window.geometry(f"{width}x{height}+{x}+{y}")
        panel = ttk.Frame(window, padding=24, style="Panel.TFrame")
        panel.pack(fill="both", expand=True, padx=10, pady=10)
        ttk.Label(
            panel, text="ADDING CRAFT COMPONENTS", style="Title.TLabel",
            font=("Segoe UI Semibold", 13),
        ).pack(anchor="w")
        ttk.Label(
            panel,
            text=f"Building and validating {len(identifiers)} unique material stacks…",
            style="Muted.TLabel",
        ...
        progress.start(10)
        window.grab_set()
        self._craft_progress_window = window
        self._craft_progress_bar = progress
        self.status_var.set(
            f"WORKING  •  building {len(identifiers)} verified material stacks…"
        )
        ...
        def worker() -> None:
            try:
                working = DyingLightSave(active_save.path, bytes(active_save.decoded))
                working.validate_with_serializer()
                added = working.spawn_named_inventory_items(
                    identifiers, quantity=MAX_QUANTITY
                )
                working.validate_with_serializer()
                result = ("ok", working, added, len(identifiers))
            except (ValueError, SaveFormatError, OSError, json.JSONDecodeError) as exc:
                result = ("error", exc, 0, len(identifiers))
            self._craft_result_queue.put(result)

        threading.Thread(target=worker, daemon=True).start()
        self.after(60, self._poll_craft_component_result)

    def _poll_craft_component_result(self) -> None:
        try:
            status, payload, added, total = self._craft_result_queue.get_nowait()
        except queue.Empty:
            self.after(60, self._poll_craft_component_result)
            return

        progress = getattr(self, "_craft_progress_bar", None)
        window = getattr(self, "_craft_progress_window", None)
        if progress is not None:
            progress.stop()
        if window is not None and window.winfo_exists():
            window.grab_release()
            window.destroy()
        self._craft_spawning = False
        self.craft_bulk_button.configure(
            state="normal", text="ADD 999,999 CRAFT COMPONENTS"
        )
        self.save_button.configure(state="normal")

        if status == "error":
            self._refresh_spawner_rows()
            self.status_var.set("FAILED  •  craft components were not changed")
            messagebox.showerror(APP_TITLE, str(payload), parent=self)
            return
        ...
        self._refresh_spawner_rows()
        self.status_var.set(
            f"STAGED  •  {total} craft components × {MAX_QUANTITY:,}  •  "
            "Press SAVE CHANGES to write"
        )
        messagebox.showinfo(
            APP_TITLE,
            f"All {total} craft components are staged at {MAX_QUANTITY:,} each.\n\n"
            f"Added {added} missing records and updated {updated} existing records.\n\n"
            "Press SAVE CHANGES to write them and create a backup.",
            parent=self,
        )
    ...
    def _max_extreme_legend_bonuses(self) -> None:
        self._set_all_legend_bonuses(32_767)

    def _reset_legend_bonuses(self) -> None:
        self._set_all_legend_bonuses(25)

    def _set_all_legend_bonuses(self, rank: int) -> None:
        if not self.save:
            messagebox.showerror(APP_TITLE, "Open a save first.", parent=self)
            return
        identifiers = (
        ...
        if not messagebox.askyesno(
            APP_TITLE,
            f"Set all 10 Legend specialization bonuses to rank {rank:,}?\n\n"
            "This includes every weapon bonus, stamina, maximum health, health "
            "regeneration and healing efficiency. The overall Legend level and XP "
            "are left unchanged.\n\n"
            "The changes remain staged until you press SAVE CHANGES.",
            parent=self,
        ):
            return
        try:
            self.save.update_skill_levels({name: rank for name in identifiers})
            self.skill_levels = self.save.read_skill_levels()
        except (SaveFormatError, OSError, json.JSONDecodeError) as exc:
            messagebox.showerror(APP_TITLE, str(exc), parent=self)
            return
        ...
        self._refresh_information_pages(refresh_skills=False)
        self.status_var.set(
            f"STAGED  •  all 10 Legend bonuses set to {rank:,}  •  "
            "Press SAVE CHANGES to write"
        )
        messagebox.showinfo(
            APP_TITLE,
            f"All 10 Legend specialization bonuses are staged at rank {rank:,}.\n\n"
            "Press SAVE CHANGES to write them and create a backup.",
            parent=self,
        )
    ...
    def _install_developer_menu(self) -> None:
        self._install_runtime_file(
            "SelvenikDevMenu.pak", Path("DW") / "Data3.pak",
            "Developer Menu (Safe Base)",
        )
    ...
    def _refresh_runtime_status(self) -> None:
        ...
        checks = (
            (
                "Dev menu safe base", game / "DW" / "Data3.pak",
                "SelvenikDevMenu.pak",
            ),
            (
    ...
    def edit_item_values(self, identifier: str) -> None:
        ...
        uses_durability = category in {"CategoryType_Melee", "CategoryType_Weapon"}
        ...
        fields = [
            ("Quantity", "quantity"),
            ("Current durability (game may clamp to source maximum)", "condition"),
        ...
        def apply_values() -> None:
            try:
                ...
                condition_value = float(variables["condition"].get())
                if uses_durability and not 0 <= condition_value <= 9_999_999:
                    raise ValueError(
                        "Melee durability must be between 0 and 9,999,999. "
                        "Negative values mark durability-based weapons as broken."
                    )
                values = {
